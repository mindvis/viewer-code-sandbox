...
// const skills = [''];
const ThreeDModelEdit = ({ forgot }: { forgot?: string }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(process.env.REACT_APP_API_URL + '/api/user/admin/fetch/'+id, {
          headers: {
  ...
  return (
    <MainCard content={false} title="Update User" sx={{ '& .MuiInputLabel-root': { fontSize: '0.875rem' } }}>
<Box sx={{ p: 2.5 }}>
    ...
    {userData && <Formik
        ...
        validationSchema={Yup.object().shape({
          ...
          confirm: Yup.string()
          .test('confirm', `Passwords don't match.`, function(value) {
          ...
          try {
            ...
            await userUpdate(values.id, values.name, values.email, values.designation, values.phone, values.phone_ext, values.role, values.password, values.confirm);       
                ...
                <Grid item xs={12} sm={6}>
                  <Stack spacing={1.25}>
                    ...
                    {touched.role && errors.role && (
                      <FormHelperText error id="user-role-helper">
                        {errors.role}
                      </FormHelperText>
                    )}
                  </Stack>
...
export default ThreeDModelEdit;
